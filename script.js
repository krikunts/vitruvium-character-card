...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function init() {
        ...
        const formWizardInstance = new FormWizard(
            formWizard,
            formTitle,
            prevBtn,
            nextBtn,
        ...
        addCategoryBtn.addEventListener('click', () => formWizardInstance.addCategoryAndNavigate(
        ...
        if (loadFileInput) loadFileInput.addEventListener('change', (event) => loadDataFromJsonHandler(event, formWizardInstance));
        if (uploadPngBtn) uploadPngBtn.addEventListener('click', () => uploadPngInput.click());
        if (uploadPngInput) uploadPngInput.addEventListener('change', (event) => uploadCardFromPng(event, formWizardInstance));
        if (prevBtn) prevBtn.addEventListener('click', () => {
            dataManager.setCurrentPageIndex(formWizardInstance.navigate(-1));
        });
        if (nextBtn) nextBtn.addEventListener('click', () => {
            dataManager.setCurrentPageIndex(formWizardInstance.navigate(1));
        });
        if (themeSelect) themeSelect.addEventListener('change', (event) => handleThemeChange(event, characterCard, jsonStyleContainer, jsonInputArea, applyStylesFromJson));
    ...
    function loadDataFromJsonHandler(event, formWizardInstance) {
        ...
        loadDataFromJson(file, (data) => applyLoadedData(data, formWizardInstance), (error) => {
            console.error('Ошибка обработки JSON файла:', error);
            notify('error', 'Ошибка при загрузке файла. Убедитесь, что это корректный JSON файл.');
        });
    ...
    function uploadCardFromPng(event, formWizardInstance) {
        const file = event.target.files[0];
        loadCardFromPng(file, (data) => applyLoadedData(data, formWizardInstance), (error) => {
            if (error.message && error.message.includes('No character data')) {
                notify('error', 'В этом PNG файле не найдено данных персонажа.');
            } else {
                notify('error', 'Ошибка при обработке PNG файла. Убедитесь, что это корректный PNG файл.');
            }
    ...
    function applyLoadedData(data, formWizardInstance) {
        ...
        if (data.styles) {
            if (data.styles.theme === 'custom') {
                ...
                const characterCard = cardWrapper.querySelector(SELECTORS.characterCard);
                handleThemeChange({ target: { value: data.styles.theme } }, characterCard, jsonStyleContainer, jsonInputArea, applyStylesFromJson);
            } else {
                setTheme(data.styles.theme);
                if (themeSelect) themeSelect.value = data.styles.theme;
                const characterCard = cardWrapper.querySelector(SELECTORS.characterCard);
    ...
    const debouncedUpdateUI = window._ ? _.debounce(() => updateUI(cardWrapper, dataManager, templates), 100) : (...args) => updateUI(cardWrapper, dataManager, templates);
    ...
    function handleFormClick(e, formWizardInstance) {
        ...
        if (button.classList.contains('remove-category-btn')) {
            formWizardInstance.removeCategoryAndUpdate(
                catId,
                () => updateUI(cardWrapper, dataManager, templates)
            );
            ...
            if (category) {
                ...
                page.querySelector(SELECTORS.skillsInCategoryContainer).appendChild(createSkillFormBlock(newSkill, templates));
                debouncedUpdateUI();
            }
